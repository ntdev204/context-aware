...
_Kết quả:_ Bạn sẽ nhận được file `models/yolo/yolo11s.engine` đã được tối ưu.
...
> **Lưu ý:** `make server-up` sẽ chạy môi trường Docker. Docker đã cài đặt sẵn hệ thống kết nối SSH ngầm ở Cổng `2222` và gắn thẳng ổ ảo `roi_incoming` cho Jetson ném file vào, bạn không cần cài thêm bất cứ tính năng phụ nào trên Windows!
...
1. **Laptop Windows**:
   Bạn chạy lệnh `make server-logs` để giám sát xem có batch nào về hay đang đào tạo AI gì không.
2. **Jetson**:
