    ...
    def run(self) -> str:
        ...
        for img in self.images:
            ...
            if not path.exists() or path.stat().st_size == 0:
                issues.append({"file": img["filename"], "reason": "empty_or_missing", "path": img["path"]})
                continue
                
            cv_img = cv2.imread(str(path))
            if cv_img is None:
                issues.append({"file": img["filename"], "reason": "corrupt_image", "path": img["path"]})
                continue
            ...
            if h in hashes:
                issues.append({"file": img["filename"], "reason": "duplicate", "path": img["path"], "duplicate_of": hashes[h]})
            else:
        ...
        report: dict[str, Any] = {
            ...
            "corrupt_images": [i for i in issues if i["reason"] == "corrupt_image" or i["reason"] == "empty_or_missing"],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="intent_dataset", help="Path to intent_dataset directory")
    parser.add_argument("--output", type=str, default=".", help="Where to save the reports directory")
