        ...
        for key in sorted(f.keys()):
            ...
            try:
                frames.append({
                    ...
                    "distance_sources": [s.decode() if isinstance(s, bytes) else s
...
def _statistics(frames: list[dict]) -> dict:
    ...
    depth_frames = sum(
        1 for f in frames
        if any(s == "depth" for s in f["distance_sources"])
    )
...
def _export_clean(frames: list[dict], issues: list[dict], out_dir: Path, session_id: str) -> None:
    ...
    bad_keys = {i["key"] for i in issues
                if any(f in ("ZERO_OBSERVATION", "NONFINITE_OBSERVATION", "DUPLICATE_FRAME_ID")
                       for f in i["flags"])}
    ...
    try:
        with h5py.File(out_path, "w", locking=True) as f:
            for i, frame in enumerate(clean):
                ...
                grp.attrs["session_id"] = frame["session_id"] if isinstance(frame["session_id"], bytes) \
                ...
                grp.create_dataset("image_jpeg",       data=np.frombuffer(frame["image_jpeg"], dtype=np.uint8), dtype="u1")
                ...
                grp.create_dataset(
                    ...
                    data=[s.encode("utf-8") if isinstance(s, str) else s
                          for s in frame["distance_sources"]] or [b"bbox"],
                    dtype=str_dtype,
                )
    finally:
        gc.enable()
...
def main() -> None:
    ...
    parser.add_argument("--samples",        type=int, default=0,   help="Save N sample images as a contact sheet")
    parser.add_argument("--export-clean",   type=Path, default=None, help="Export clean frames to this directory")
    parser.add_argument("--json",           action="store_true",   help="Print report as JSON (machine-readable)")
    ...
    print(f"\n{'='*60}")
    print(f"  Experience Data Explorer")
    ...
    for h5_path in files:
        ...
        if args.json:
            ...
            print(f"  Duration   : {stats.get('duration_s', 0):.1f}s  @ {stats.get('avg_fps', 0):.1f} FPS")
            print(f"  Obs dim    : {stats.get('obs_dim')}   mean={stats.get('obs_mean', 0):.3f}  std={stats.get('obs_std', 0):.3f}  NaN={stats.get('obs_has_nan')}")
            print(f"  Depth cov  : {stats.get('depth_frame_pct', 0):.1f}% frames with real depth")
            print(f"  Detections : {stats.get('person_detections', 0)} person detections  avg_dist={stats.get('avg_person_distance_m')}m")
    ...
    if len(files) > 1:
        ...
        print(f"  Clean frames : {clean_pct:.1f}%  ({len(all_frames)-len(all_issues)}/{len(all_frames)})")
